...
- **实战目标课程**：改为从 `dept_course_selections.practical_selections` + `ai_course_planning_info` 的“实战”级别取。
- **实战已完课数据**：改为从 `hands_on_courses`（工号 + task_status=finished）联表 `ai_course_planning_info`（`hands_on_courses.task_type = ai_course_planning_info.syb_type`）得到完课课程 ID（不再从 `t_micro_study_info_sync` / `t_mooc_study_info_sync` 的 course_team_code 取）。
...
| 2 | **若部门有选课**：即 `practical_selections` 非空且解析后 ID 列表不为空。用这些课程 ID 查询 **`ai_course_planning_info`**，并筛选 `course_level = '实战'`，得到实战目标课程列表（字段至少包含：**id**、**syb_type**、**course_name**）。 |
| 3 | **若部门无选课或为空**：即未配置或 `practical_selections` 为空。则从 **`ai_course_planning_info`** 取 `course_level = '实战'` 的**全部课程**，作为实战目标课程列表。 |
...
| 2 | 用上述 `task_type` 列表与 **`ai_course_planning_info`** 联表（或 IN 查询）：通过 `ai_course_planning_info.syb_type` 与 `hands_on_courses.task_type` 对应，筛选 `course_level='实战'`，查询得到这些已完课任务对应的 **课程主键 id**。 |
...
- 若 `hands_on_courses` 中某 `task_type` 在 `ai_course_planning_info`（syb_type）中无对应“实战”记录，则该条不产生课程 id，不写入 `practical_courses`。

### 3.3 practical_courses 字段的写入

- **内容**：该员工在「实战目标课程列表」范围内、且已在 `hands_on_courses` 中完课（task_status=finished）所对应的 **`ai_course_planning_info.id`**，**逗号分隔**，升序排序，写入 `t_employee_training_info.practical_courses`。
...
| 表名 | 用途 | 关键字段 / 说明 |
|------|------|------------------|
| **dept_course_selections** | 部门选课配置 | 现有：dept_code, course_selections, basic/advanced/practical_target_courses_num。<br>**新增**：**practical_selections**（实战课程 ID 逗号分隔，对应 ai_course_planning_info.id）。 |
| **ai_course_planning_info** | 课程主数据 | 本次用于**基础/进阶**与**实战目标/完课映射**。其中实战课程通过 `course_level='实战'` 识别；完课映射通过 `syb_type` 与 hands_on_courses.task_type 对应。 |
...
getDeptSelectionByDeptCode(fourthDeptCode) → practical_selections
    ↓
有选课？── 是 → 用 ID 列表查 ai_course_planning_info(course_level='实战') → 实战目标课程列表
    └── 否/空 → 查 ai_course_planning_info(course_level='实战') → 实战目标课程列表
    ↓
hands_on_courses: account + task_status='finished' → task_type 列表
    ↓
task_type 联表 ai_course_planning_info（syb_type） → 得到已完课课程 id 集合
    ↓
...
3. **CoursePlanningInfoMapper / PersonalCourseCompletionMapper**
   - 实战目标课程从 `ai_course_planning_info` 中按 `id IN practical_selections` 且 `course_level='实战'` 获取；无选课则直接取 `course_level='实战'` 的全量课程。

4. **HandsOnCourseMapper**
   - 按**工号** + **task_status = 'finished'** 查询 `hands_on_courses`，返回 **task_type** 列表（去重）。
   - 或：按工号 + task_status=finished 联表 `ai_course_planning_info`（`h.task_type = p.syb_type` 且 `p.course_level='实战'`），直接返回**已完课对应的课程 id 列表**。
...
- **dept_course_selections**：若表中尚无 `practical_selections` 字段，需执行：
  ```sql
  ALTER TABLE dept_course_selections ADD COLUMN practical_selections VARCHAR(500) NULL COMMENT '实战课程ID逗号分隔，对应ai_course_planning_info.id' AFTER course_selections;
  ```
- **ai_course_planning_info**：需确保实战课程具备 `course_level='实战'` 且可用 `syb_type` 与 hands_on_courses.task_type 对应。
